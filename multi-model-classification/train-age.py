...
train_dataset = AgeDataset(train, transform_train, train=True)
train_set = DataLoader(train_dataset, batch_size=BATCH_SIZE,shuffle=True)

test_dataset = AgeDataset(test, transform_test, train=True)
...
def train(epoch):
    ...
    if epoch%5==0:
        torch.save({
          ...
          }, f"/opt/ml/result/checkpoint/age_resnet18_e_{epoch}_{loss}.pt")
    ...
    with torch.no_grad():
        ...
        if acc<=100.*correct/total:
            ...
            torch.save({
              ...
              "acc": 100*correct/total
              }, f"/opt/ml/result/checkpoint/age_resnet18_best_performance_{acc}_{loss}.pt")
